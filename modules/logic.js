...
class Calculation{
    constructor(){
        ...
        this.mainArray=[];
        this.DOMdisplay=document.createElement('section');
        this.DOMdisplay.style.border='2px solid';
        this.DOMdisplay.style.margin='3px';
        this.DOMdisplay.style['border-radius']='10px';
        this.DOMdisplayCalculation=document.createElement('p');
        this.DOMdisplayResult=document.createElement('p');
        this.DOMdisplay.appendChild(this.DOMdisplayCalculation);
        this.DOMdisplay.appendChild(this.DOMdisplayResult);
        this.currDomEle;
    }
    set(val){
    ...
    display(){
        ...
        let theElement;
        let theHistory;
        function ele(boolean){
            let span=boolean;
            let element;
            let supM1=document.createElement('sup');
            supM1.appendChild(textNode('-1'));
            {
            if(span){
                element=eleNode().span;
            }
            else element = eleNode().sup;
        }
            switch (currentValue){
                case 'sin': element.appendChild(textNode('sin')) ; break;
                case 'cos': element.appendChild(textNode('cos')); break;
                case 'tan': element.appendChild(textNode('tan')); break;
                case 'ln': element.appendChild(textNode('ln')); break;
                case 'log': element.appendChild(textNode('log')); break;
                case 'arcSin': element.appendChild(textNode('sin')); element.appendChild(supM1); break;
                case 'arcCos': element.appendChild(textNode('cos')); element.appendChild(supM1); break;
                case 'arcTan': element.appendChild(textNode('tan')); element.appendChild(supM1); break;
                case '10powX': element.appendChild(textNode('10')); break;
                case 'epowX': element.appendChild(textNode('e')); break;
                case 'xpow2': element.appendChild(textNode('2')); element.setAttribute('square',''); break;
                case 'percentage':element.appendChild(textNode('% ')) ; break;
                case 'sqrt': element.appendChild(textNode('√')) ; break;
                case 'powY': element.appendChild(textNode('')); break;
                case 'rtY': element.appendChild(textNode('√')); break;
                case 'pie': element.appendChild(textNode('π')); break;
                case 'n-factorial':element.appendChild(textNode('!')) ; break;
                case '(': element.appendChild(textNode('(')); break;
                case ')': element.appendChild(textNode(')')); break;
                case 'plusOrMinus': element.appendChild(textNode('±')); break;
                case '7': element.appendChild(textNode('7')) ; break;
                case '8': element.appendChild(textNode('8')); break;
                case '9': element.appendChild(textNode('9')); break;
                case '4': element.appendChild(textNode('4')); break;
                case '5': element.appendChild(textNode('5')); break;
                case '6': element.appendChild(textNode('6')); break;
                case '1': element.appendChild(textNode('1')); break;
                case '2': element.appendChild(textNode('2')); break;
                case '3': element.appendChild(textNode('3')); break;
                case '.': element.appendChild(textNode('.')); break;
                case '0': element.appendChild(textNode('0')); break;
                case '+': element.appendChild(textNode('+')); break;
                case '-': element.appendChild(textNode('-')); break;
                case '/': element.appendChild(textNode('÷')); break;
                case '*': element.appendChild(textNode('×')); break;
        ...
        function eleNode(){
            ...
            return {sup:s,span:sp};
        }
        function textNode(str){
            return document.createTextNode(str);
        }
        if(this.previousValue==undefined){
            if(this.currentValue=='xpow2'){
               theElement= ele(false);
               theHistory=ele(false);
            }
        else{ 
            theElement= ele(true);
            theHistory=ele(true);
           ...
           if(['10powX','epowX','powY'].some(x=> x==this.previousValue)){
            theElement = ele(false);
            theHistory=ele(false);
           }
           else if(['+','-','/','*'].some(x=>x==this.currentValue)){theElement=ele(true); theHistory=ele(true)}
           else if((textInputBlock.lastElementChild.localName=='sup' && typeof Number(currentValue) !== 'number') && !textInputBlock.lastElementChild.hasAttribute('square')){
           theElement = ele(false);
           theHistory= ele(false);
           }
           else if(this.currentValue=='xpow2'){
           theElement = ele(false);
           theHistory= ele(false);
           }
           else{
        ...
        this.DOMdisplayCalculation.appendChild(theHistory);
        textInputBlock.appendChild(theElement);
    }
    delete(){
        if(this.mainArray.length !== 0){
        this.mainArray.pop();
        textInputBlock.removeChild(textInputBlock.lastElementChild);
        this.DOMdisplayCalculation.removeChild(this.DOMdisplayCalculation.lastElementChild);
        if(this.mainArray.length==0){
            this.currentValue=undefined;
            this.previousValue=undefined;
        }
        ...
        else this.previousValue=this.mainArray[this.mainArray.length-2];
...
get compliant(){
    let functionalKeys = ['sin','cos','tan','ln','log','arcSin','arcCos','arcTan','10powX','epowX','xpow2','percentage','sqrt','powY'];
    ...
    function protocol1(){
        ...
        let ErrorMessage = 'Syntax : Multiply or divide sign inappropriately used.';
      
        function protocol1test(current,next){
            let combined=[current].concat([next]);
            combined= combined.join('');
            if((combined.indexOf('*/')!==-1 || combined.indexOf('/*')!==-1) || (combined.indexOf('//')!==-1 || combined.indexOf('**')!==-1)){
              return false;
            }
            else return true;
          }
    ...
    function protocol2(){
        ...
        let ErrorMessage='Syntax: Calculation cannot begin with ' + workingArray[0];
        
        if(['*','/','powY','.','root','xpow2','n-factorial','percentage'].some(x=> x==workingArray[0])){
    ...
    function protocol3(){
        ...
        let ErrorMessage='Syntax: Calculation Cannot end with a ' + workingArray[workingArray.length-1];
        if(['*','/','+','-','.','sin','cos','tan','ln','log','arcSin','arcCos','arcTan','10powX','epowX','percentage','sqrt','powY','rtY',,'openBracket','plusOrMinus'].some(x=> x==workingArray[workingArray.length-1])){
    ...
    function protocol4(){
        ...
        let ErrorMessage='Syntax: Wrong placement of first character in bracket.';
        
        function protocol4test(current,next){
            let combined=[current,next];
                  combined= combined.join('');
        if(['(*','(/','(powY','(.','(root','(xpow2','(n-factorial','(percentage'].some(x=> x==combined)){
          return false;
        }
        else return true;
      }
    ...
    function protocol5(){
        ...
        let ErrorMessage='Syntax: bracket ended with an inapppropraite character';
        
        function protocol5test(current,next){
            let combined=[current].concat([next]);
                  combined= combined.join('');
            if(['*)','/)','+)','-)','.)','sin)','cos)','tan)','ln)','log)','arcSin)','arcCos)','arcTan)','10powX)','epowX)','percentage)','sqrt)','powY)','rtY)','plusOrMinus)'].some(x=> x==combined)){
                return false;
            }
            return true;
        }
    ...
    function protocol6(){
        ...
        function basicAssert(input){
            ...
            let extracted=[];
            let openbracketCount=0; let closedbracketCount=0;
            for(let each of input){
                if(each =='(' || each ==')'){
                    extracted.push(each);
        ...
        function deepAssert(input){
            let openbracketCount=0,closedbracketCount=0; let extracted=[];
            for(let each of input){
                if(each=='(' || each==')'){
                    extracted.push(each);
                }
    ...
    function protocol7(){
        ...
        let ErrorMessage='Only a number or a bracket can follow a root character';
    ...
    function protocol8(){
       ...
       let ErrorMessage='Expressions that\' are not numbers when followed a functional key should be put in bracket';
       function protocol8test(current,next){
        if(functionalKeys.some(x=> x==current && ( x !== 'xpow2' && x !== 'n-factorial' && x!=='pie')) && functionalKeys.concat(['*','/','-','+',')']).some(x=> x==next)){
            return false;
        }
        else return true;
       }
    ...
    function protocol13(){
        ...
        function protocol13test(first,second,third){
            if(second == '.' && (typeof Number(first) !== 'number' && typeof Number(third) !== 'number')){
                return false;
            }
            else return true;
        }
        if(!check3(workingArray,protocol13test)){  throw new protocol13Error(ErrorMessage);}
        else return true;
    }
    function protocol14(){
        ...
        let ErrorMessage='Inappropraite placement of character after a plus or minus';
        function protocol14test(current,next){
        if(['+','-'].some(x=> current==x) && ['*','/','powY','.','root','xpow2','n-factorial','percentage'].some(x=> x==next)){
            return false;
        }
        return true;
        }
    ...
    let result = true;
    let protocolArray=[protocol1,protocol2,protocol3,protocol4,protocol5,protocol6,protocol7,protocol8,protocol13,protocol14];
...
get modify(){
    ...
    let mod=[]; let str=''; let pushed=false;
    ...
    for(let each of arr){
      ...
      if(!prevprev){
        ...
        if((prev=='+' || prev=='-') && ['*','/','(',')'].some(x=>x==prevprev)){
          modArr[modArr.length-1]+=each;
          prev=modArr[modArr.length-1];
        }
        else{
...
function SD(str){
  ...
  function removeMinusAndPlusSequence(arr){
    ...
    if(start && end){
        ...
        let deleteCount=end+1-start; arr.splice(start,deleteCount,sign);
    }
    if(extract(arr)){
...
modified=minusEval(modified);
 ...
 getResult(array){
    ...
    let rf=['10powX','epowX'];
    let amS=['sin','cos','tan','ln','log','arcSin','arcCos','arcTan','percentage','rtY','pie','sqrt','powY'];
...
if(prev && next){
    ...
    if(hasBracket()){
       ...
       if(Number(calculation[extracted.start-1]) !== NaN){
        calculation.splice(extracted.start-1,(extracted.end+1)-(extracted.start-1),result*calculation[extracted.start-1])
       }
        else{
